...
namespace Do_Nation
{
    public partial class DonationList : PhoneApplicationPage
    {
        public DonationList()
        {
            InitializeComponent();
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            int x = donationList.Children.Count;
            for (int i = 0; i < x; i++)
                donationList.Children.RemoveAt(0);
            loadDonations();
        ...
        private async void loadDonations()
        {
            ...
            foreach (Post p in donations)
            {
                if (p.status == "NEW")
                {
                    ...
                    donationList.Children.Add(postPanel);
                }
                
            }

            if (donationList.Children.Count == 0)
                donationList.Children.Add(new noneChild("No available donations."));
        }
