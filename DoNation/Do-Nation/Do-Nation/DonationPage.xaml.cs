...
namespace Do_Nation
{
    public partial class DonationPage : PhoneApplicationPage
    {
        string donationid = "";
        string donatorID = "";
        string donatorName = "";
        ...
        protected override void OnNavigatedTo(System.Windows.Navigation.NavigationEventArgs e)
        {
            try
            {
                ...
                donatorID = NavigationContext.QueryString["param1"];
            }
            catch (Exception) { }
            loadContactDetails();
            loadDonationDetails();
            ...
            foreach (Post p in donationList)
            {
                if (p.id.Equals(donationid))
                {
                  ...
                  //donatorID = p.userid;
                    ...
                    if (!statustb.Text.Equals("NEW"))
                        btn_Request.Visibility = System.Windows.Visibility.Collapsed;
                    break;
                }
            }
        }
        ...
        private async void requestDonation(object sender, System.Windows.RoutedEventArgs e)
        {
            blackeffect.Visibility = Visibility.Visible;
            progressBar.Visibility = Visibility.Visible;

            Notification notif = new Notification();
            notif.isreadbyvolunteer = false;
            notif.postid = donationid;
            notif.status = "PENDING";
            updatePostStatus("PENDING FOR APPROVAL", donationid);
            notif.userid = donatorID;
            notif.volunteerid = MainPage.userID;
            await App.MobileService.GetTable<Notification>().InsertAsync(notif);
